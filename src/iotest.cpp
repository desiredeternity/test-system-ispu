...
// Функция для удаления незначащих пробелов и замены переносов на пробелы
...
// Запуск программы пользователя с определёнными входными данными. Используется перенаправление потоков cin/cout и stdin/stdout
...
// Парсинг чисел double из string
...
// Тестирование на полное сходство ожидаемого и реального вывода программы
...
// Тестирование на полное сходство ожидаемого и реального вывода программы, несколько возможных вариантов
...
// Тестирование на наличие необходимых чисел в выводе программы, сохранение порядка может быть выбрано
void test_num_array(std::string input, const char* expected, bool ignore_order) {
    std::string result = run_with_input(input);
...
// Тестирование на наличие необходимых чисел в выводе программы, сохранение порядка может быть выбрано, несколько возможных вариантов
void test_num_array(std::string input, std::vector<std::string> expected_list, bool ignore_order) {
    std::string result = run_with_input(input);
    std::vector<double> num_result = dfromstr(result);
...
// Тестирование на наличие определённой подстроки в выводе программы
...
// Тестирование на наличие хотя бы одной из подстрок в выводе программы
